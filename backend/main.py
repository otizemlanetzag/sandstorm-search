...
app = FastAPI(title="Sandstorm Search", version="0.5.0")
...
def search_rows(q: str, limit: int = 10):
    conn = db()
    try:
...
@app.get("/api/search")
def search_api(q: str = Query(min_length=1, max_length=300), limit: int = Query(10, ge=1, le=50)):
    return {"query": q, "results": [dict(r) for r in search_rows(q, limit)]}


@app.get("/search", response_class=HTMLResponse)
def search_page(q: str = Query(default="", max_length=300)):
    results = search_rows(q, 20) if q.strip() else []
    cards = "".join(
    ...
    if q.strip() and not results:
        cards = '<p class="empty">No indexed results yet.</p>'
    return HTMLResponse(PAGE.replace("__QUERY__", html.escape(q)).replace("__RESULTS__", cards))
...
<form class="search" method="get" action="/search"><input id="q" name="q" value="__QUERY__" autofocus placeholder="Search the web..."><button type="submit">Search</button></form><main id="results">__RESULTS__</main>
